...
## v0.4.1 - 2026-06-10 — Private → public bridge

### Added

- **Bridge script** (`scripts/sync_from_private.py`): automated, sanitized delta report between a private production library and this public template. Names only generic class-level files; domain/private items appear as anonymous counts; the report itself must pass `sanitize_check.py` or nothing is written. State in `.private-sync-state.json` keeps subsequent runs delta-only. Closes open question #6 ("private → public bridge") from the production concept audit.
- **Sync status report** (`docs/private-sync-status.md`): generated by the bridge; shows what generic capabilities exist upstream with no template counterpart yet, so porting (always a deliberate, generalized rewrite) is visible and cheap to schedule.
- **Runbook → consumer wiring runbook** (`library-template/wiki/Runbooks/runbook-consumer-wiring.md`): distilled from production — a runbook only instructs if some operating surface actually loads it; audit consumers, pin pointers (<400 chars) into profile prompts, archive what has no living consumer, and make lint a daily metric instead of a script nobody runs.
